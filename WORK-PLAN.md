...
- **3D hand visualization (replace skeleton wireframe) -- two approaches, pick one:**
  - *Option A: Procedural primitive hand.* Build the hand directly from the 21 landmarks using spheres at joints (sized by anatomical role: MCP thicker than DIP), capsules/tubes between them, ovals for fingertip pads, ellipsoid for the palm. No external mesh, no rigging -- the hand IS the data. Derive bone lengths from landmark distances each frame, apply ratio constants for joint thickness. The craft is in tuning proportions so it reads as a hand, not a balloon animal. Zero external assets, all data-driven.
  - *Option B: Skinned mesh hand.* Source or generate a proper 3D hand mesh with a 21-bone rig matching MediaPipe's skeleton topology. Load in Three.js as a skinned mesh, drive bone transforms from landmarks each frame. More upfront work (mesh sourcing, rest-pose alignment) but potentially more polished final look. Could be generated, free CC0/CC-BY, or AI-generated.
  - Either way: skeleton wireframes read as "tech demo," a real hand reads as "product."
- **Blendshape-driven head model demo:** Source a free 3D head mesh (Ready Player Me, Sketchfab CC0, or similar) that supports ARKit-compatible blendshapes (52 shapes). Our face pipeline already outputs blendshapes via `face-blendshape-worker.js` -- this demo wires those weights directly into the mesh's morph targets so the 3D head mirrors the user's facial expressions in real time. Bonus: experiment with WebGPU blend modes for rendering (additive glow on eyes, multiply for shadows, screen for highlights) to make the demo visually striking beyond just "face puppet." Steps: (1) find a head mesh with blendshape support (Apple's ARKit blendshape naming convention is the standard; many free models use it), (2) load in Three.js with morph targets, (3) map our 52 blendshape outputs to the mesh's morph target influences each frame, (4) add blend-mode rendering passes for visual flair. This is the face pipeline's equivalent of the hand mesh upgrade -- proves the blendshapes actually work by showing a face that moves.
