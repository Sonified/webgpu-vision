...
@group(0) @binding(4) var<storage, read> prelu_slope: array<f16>;
@group(0) @binding(5) var<storage, read> residual: array<f16>;
@group(0) @binding(6) var<storage, read_write> output: array<f16>;

var<workgroup> tile: array<f16, 400>;
    ...
    // === 1x1 POINTWISE PATH ===
        ...
        for (var oc_t: u32 = 0u; oc_t < params.oc_tile; oc_t++) {
            ...
            for (var ic8: u32 = 0u; ic8 < cpg8; ic8++) {
                ...
                let v0 = vec4<f16>(input[i0], input[i0 + stride], input[i0 + stride * 2u], input[i0 + stride * 3u]);
                let w0 = vec4<f16>(weights[w_base + ic8 * 8u], weights[w_base + ic8 * 8u + 1u], weights[w_base + ic8 * 8u + 2u], weights[w_base + ic8 * 8u + 3u]);
                let i1 = i0 + stride * 4u;
